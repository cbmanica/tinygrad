...
class CLIPTextEncoderL:
  ...
  def encode(self, tokens: Tensor) -> Tensor:
    ...
    eos_pos = (tokens == 49407).int().argmax(dim=-1)  # position of EOT token
    pooled = x[Tensor.arange(tokens.shape[0]), eos_pos]  # (B, 768)
    return self.text_projection(pooled)  # (B, 768)
...
def make_txt_ids(seq_len: int, batch: int) -> Tensor:
  i_idx = Tensor.arange(seq_len, dtype=dtypes.int32).reshape(1, seq_len).expand(batch, seq_len)
  zeros = Tensor.zeros(batch, seq_len, dtype=dtypes.int32)
  return Tensor.stack(zeros, zeros, i_idx, dim=-1)
...
def encode_prompt(prompt: str, model_dtype) -> tuple:
  ...
  del t5
  print(f"  t5_emb: {t5_emb.shape}, VRAM: {GlobalCounters.mem_used/1e9:.2f} GB")
  ...
  del clip
  print(f"  clip_pooled: {clip_pooled.shape}, VRAM: {GlobalCounters.mem_used/1e9:.2f} GB")
...
def euler_sample(model: Flux, latents: Tensor, img_ids: Tensor,
  ...
  for i in trange(num_steps):
    with Timing("  step: ", enabled=timing, on_exit=lambda _: f", VRAM {GlobalCounters.mem_used/1e9:.2f} GB"):
      ...
      t_vec  = Tensor.full((latents.shape[0],), t_curr, dtype=latents.dtype)

      if model_name == "flux-dev":
        g_vec = Tensor.full((latents.shape[0],), guidance_scale, dtype=latents.dtype)
        velocity = model(latents, img_ids, t5_emb, txt_ids, t_vec, clip_pooled, g_vec)
      else:
        velocity = model(latents, img_ids, t5_emb, txt_ids, t_vec, clip_pooled)
